...
impl Pipeline {
    ...
    pub async fn run(&mut self) -> CarbonResult<()> {
        log::info!(
            "starting pipeline. num_datasources: {}, num_account_pipes: {}, num_account_deletion_pipes: {}, num_instruction_pipes: {}, num_transaction_pipes: {}",
                ...
                update = update_receiver.recv() => {
                    match update {
                        Some((update, datasource_id)) => {
                            if let Err(error) = self.process(update.clone(), datasource_id.clone()).await {
